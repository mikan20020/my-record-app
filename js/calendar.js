...
function getCalendarRecord(type, id) {
  ...
  return sources[type]?.().find((record) => record.id === id) || null;
}
...
function editorButtons() {
  return `
    <div class="calendar-editor-actions">
      ...
      <button type="button" class="cancel-edit-button calendar-edit-cancel">キャンセル</button>
    </div>`;
}
...
function deleteCalendarRecord(type, id) {
  const handlers = {
    ...
    medicine: [loadMedicineRecords, saveMedicineRecords],
    caffeine: [loadCaffeineRecords, saveCaffeineRecords],
    outing: [loadOutingRecords, saveOutingRecords],
    housework: [loadHouseworkRecords, saveHouseworkRecords]
  };

  const [load, save] = handlers[type] || [];
  if (!load || !save) return;
  save(load().filter((record) => record.id !== id));
  refreshAfterCalendarChange(selectedCalendarDate);
  if (
  window.firebaseSync &&
  typeof window.firebaseSync.queueUpload === "function"
) {
...
selectedDateRecords.addEventListener("click", (event) => {
  const editButton = event.target.closest(".calendar-edit-button");
  if (editButton) {
    startCalendarEditing(editButton.dataset.calendarType, editButton.dataset.calendarId);
    return;
  }

  const deleteButton = event.target.closest(".calendar-delete-button");
  if (deleteButton) {
    if (confirm("この記録を削除しますか？")) {
      deleteCalendarRecord(deleteButton.dataset.calendarType, deleteButton.dataset.calendarId);
    }
    return;
  }

  if (event.target.closest(".calendar-edit-cancel") && selectedCalendarDate) {
    displaySelectedDateRecords(selectedCalendarDate);
  }
});

selectedDateRecords.addEventListener("submit", (event) => {
  const form = event.target.closest(".calendar-inline-editor");
  if (!form) return;
